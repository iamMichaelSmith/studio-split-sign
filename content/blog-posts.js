...
const blogUpgradeGuides = {
  "how-to-rescan-plugins-in-your-daw": {
    ...
    audience: [
      ...
      ["Artists", "If the DAW cannot load the beta, use the hosted web app so the split sheet still gets completed before collaborators leave."],
    ...
    tableHeaders: ["Host or workflow", "Current beta path", "Best next step"],
    rows: [
      ["Studio One, Ableton Live, FL Studio, Cubase, REAPER, Cakewalk/Sonar, Bitwig, Mixcraft, Waveform", "Windows VST3 hosts can usually load the beta.", "Install to the standard VST3 folder, rescan, then test in a blank project."],
...
function renderCreatorResources(limit = 11) {
  const links = [
    ...
    ["Windows VST3 beta", "https://splitsheetstudio.com/beta"],
...
const blogPosts = [
  ...
  createPost({
    ...
    sections: [
      ...
      { id: "windows-beta", title: "How the current Windows beta fits the workflow", html: `<p>Split Sheet Studio can be used through the web app, standalone Windows app, and current Windows VST3 beta. Until broader plug-in formats and Windows code signing are completed, studios can still rely on the hosted web workflow for client-facing closeout and use the VST3 beta in compatible Windows DAWs that accept VST3.</p><p>That keeps the public offer honest: the web workflow is ready for session closeout, while the downloadable DAW workflow continues improving through beta feedback.</p>` },
  ...
  createPost({
    ...
    takeaway: "A rescan cannot make an incompatible format work. Windows VST3 hosts can load the current Split Sheet Studio beta; Pro Tools requires AAX, and Logic Pro uses Audio Units on macOS.",
    sections: [
      ...
      { id: "split-sheet-studio-beta", title: "Testing the Split Sheet Studio Windows beta", html: `<p>Split Sheet Studio is currently distributed as a free 64-bit Windows VST3 beta plus a standalone app. It is designed for most Windows DAWs that support VST3 effects, but compatibility can vary by DAW version, security settings, and plug-in scanner.</p><ol><li>Close your DAW.</li><li>Download and run the beta installer.</li><li>Open your preferred VST3-compatible Windows DAW.</li><li>Rescan using the instructions above.</li><li>Load Split Sheet Studio as an effect, sign in, and run a test split.</li></ol><p>If your DAW does not host VST3, use the standalone or <a href="https://app.splitsheetstudio.com">web app</a> while native formats are expanded.</p>` }
    ],
    faqs: [
